alter table raw.googleads_webpage_criteria_conv_report add if not exists partition (data_date='{data_date}');
...
WITH api_data AS (
    ...
    FROM  
        raw.googleads_webpage_criteria_conv_report
    WHERE 
...
deactivated_customer_data AS (
    SELECT 
        *
    FROM
        mart_mops.googleads_webpage_criteria_conv_report
    WHERE
...
INSERT OVERWRITE TABLE mart_mops.googleads_webpage_criteria_conv_report
PARTITION(data_date)
select
    *
from  
