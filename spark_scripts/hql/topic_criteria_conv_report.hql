alter table raw.googleads_topic_criteria_conv_report add if not exists partition (data_date='{data_date}');
...
FROM  raw.googleads_topic_criteria_conv_report
...
deactivated_customer_data AS (
    SELECT 
        *
    FROM
        mart_mops.googleads_topic_criteria_conv_report
    WHERE
...
INSERT OVERWRITE TABLE mart_mops.googleads_topic_criteria_conv_report
PARTITION(data_date)
select
    *
from  
