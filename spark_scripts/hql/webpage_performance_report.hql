alter table raw.googleads_webpage_performance add if not exists partition (data_date='{data_date}');
...
ad_group_criterion_webpage_criterion_name as criteria,
...
FROM  raw.googleads_webpage_performance
...
deactivated_customer_data AS (
    SELECT 
        *
    FROM
        mart_mops.googleads_webpage_performance
    WHERE
...
INSERT OVERWRITE TABLE mart_mops.googleads_webpage_performance
PARTITION(data_date)
select
    *
from  
