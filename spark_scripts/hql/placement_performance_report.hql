alter table raw.googleads_placement_performance add if not exists partition (data_date='{data_date}');
...
CASE
    WHEN ad_group_criterion_type_ = 'PLACEMENT' THEN ad_group_criterion_placement_url
    WHEN ad_group_criterion_type_ = 'MOBILE_APP_CATEGORY' THEN ad_group_criterion_mobile_app_category_mobile_app_category_constant
    WHEN ad_group_criterion_type_ = 'MOBILE_APPLICATION' THEN ad_group_criterion_mobile_application_app_id
    WHEN ad_group_criterion_type_ = 'YOUTUBE_CHANNEL' THEN ad_group_criterion_youtube_channel_channel_id
    WHEN ad_group_criterion_type_ = 'YOUTUBE_VIDEO' THEN ad_group_criterion_youtube_video_video_id
    ELSE '--'
END AS criteria,
...
FROM  raw.googleads_placement_performance
...
deactivated_customer_data AS (
    SELECT 
        *
    FROM
        mart_mops.googleads_placement_performance
    WHERE
...
INSERT OVERWRITE TABLE mart_mops.googleads_placement_performance
PARTITION(data_date)
select
    *
from  
