...
FROM 
    raw.googleads_keyword_performance
where 
    ad_group_criterion_keyword_text != 'ad_group_criterion_keyword_text' 
...
deactivated_customer_data AS (
    SELECT 
        *
    FROM
        mart_mops.googleads_keyword_performance
    WHERE
...
INSERT OVERWRITE TABLE mart_mops.googleads_keyword_performance
PARTITION(data_date)
select
    *
from  
