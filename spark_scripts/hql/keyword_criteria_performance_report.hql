alter table raw.googleads_keyword_criteria_performance add if not exists partition (data_date='{data_date}');
...
FROM  raw.googleads_keyword_criteria_performance
...
deactivated_customer_data AS (
    SELECT 
        *
    FROM
        mart_mops.googleads_keyword_criteria_performance
    WHERE
...
INSERT OVERWRITE TABLE mart_mops.googleads_keyword_criteria_performance
PARTITION(data_date)
select
    *
from  
