alter table raw.googleads_parent_performance add if not exists partition (data_date='{data_date}');
...
ad_group_criterion_parental_status_type_ as criteria,
...
FROM  raw.googleads_parent_performance
...
deactivated_customer_data AS (
    SELECT 
        *
    FROM
        mart_mops.googleads_parent_performance
    WHERE
...
INSERT OVERWRITE TABLE mart_mops.googleads_parent_performance
PARTITION(data_date)
select
    *
from  
