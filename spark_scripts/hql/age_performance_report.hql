alter table raw.googleads_age_performance add if not exists partition (data_date='{data_date}');
...
ad_group_criterion_age_range_type_ as criteria,
...
FROM  raw.googleads_age_performance
...
deactivated_customer_data AS (
    SELECT 
        *
    FROM
        mart_mops.googleads_age_performance
    WHERE
...
INSERT OVERWRITE TABLE mart_mops.googleads_age_performance
PARTITION(data_date)
select
    *
from  
