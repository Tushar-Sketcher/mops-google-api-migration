alter table raw.googleads_user_performance add if not exists partition (data_date='{data_date}');
...
CASE
    WHEN ad_group_criterion_type_ = 'USER_LIST' THEN ad_group_criterion_user_list_user_list
    WHEN ad_group_criterion_type_ = 'USER_INTEREST' THEN ad_group_criterion_user_interest_user_interest_category
    WHEN ad_group_criterion_type_ = 'CUSTOM_INTENT' THEN ad_group_criterion_custom_intent_custom_intent
    WHEN ad_group_criterion_type_ = 'CUSTOM_AUDIENCE' THEN ad_group_criterion_custom_audience_custom_audience
    ELSE '--'
END AS criteria,
...
FROM  raw.googleads_user_performance
...
deactivated_customer_data AS (
    SELECT 
        *
    FROM
        mart_mops.googleads_user_performance
    WHERE
...
INSERT OVERWRITE TABLE mart_mops.googleads_user_performance
PARTITION(data_date)
select
    *
from  
