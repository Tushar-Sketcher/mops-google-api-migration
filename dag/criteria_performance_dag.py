...
def get_dag(env, config, start_date, dag_id):
    # DAG default args
    default_args = {
        'owner': "mopsde",
        'start_date': start_date,
        'email': "mops_criteria_performance_reports@zillowgroup.opsgenie.net",
...
def ValidDataSensor(env, sess_dag, brand_exp_config, hive_table):
    ...
    return ValidDataInstanceSensor(
        entity_uid=brand_exp_config['feeds']["googleads_criteria_performance"]['lcp']['entity_uid'],
        instance_name=brand_exp_config['feeds']["googleads_criteria_performance"]['lcp']['instance_name'],
        at_time='{{ ds }}',
        # at_time='{{ prev_ds }}',
        contract_group_uid=brand_exp_config['feeds']["googleads_criteria_performance"]['lcp']['contract_group_uid'],
        api_token=brand_exp_config['feeds']["googleads_criteria_performance"]['lcp']['api_token'],
        env=env,
        poke_interval=60,
        timeout=60,
        dag=sess_dag,
...
def ValidateNewDataInstanceAndPublishEvaluationsOperator(env, sess_dag, brand_exp_config, hive_table, zodiac_config):
    ...
    return ZacsValidateNewDataInstanceAndPublishEvaluationsOperator(
        ...
        api_token=brand_exp_config['feeds']["googleads_criteria_performance"]['lcp']['api_token'],
        entity_uid=brand_exp_config['feeds']["googleads_criteria_performance"]['lcp']['entity_uid'],
        hive_table=brand_exp_config['feeds']["googleads_criteria_performance"]['tables'],
        contract_group_uid=brand_exp_config['feeds']["googleads_criteria_performance"]['lcp']['contract_group_uid'],
        partition_map='{}:"{}"'.format(brand_exp_config['feeds']["googleads_criteria_performance"]['lcp']['partition_map'],
                                       '{{ ds }}'),
        ...
        driver_memory=brand_exp_config['feeds']["googleads_criteria_performance"]['lcp']['driver_memory'],
...
def publish_availability_task(env, dag, conf, task_id, hive_table):
    ...
    api_token = conf['feeds']["googleads_criteria_performance"]['lcp']["lcp_api"]
...
# dag object
the_dag = get_dag(env, config, start_date=config['start_date'], dag_id=config['dag_id'])
...
START_dummy_operator = DummyOperator(task_id="Start_Criteria_Load", dag=the_dag)

age_external_dependency = DataAvailabilitySensor(
    schema=config["feeds"]["age_performance_report"]["tables"].split('.')[0],
    table_name=config["feeds"]["age_performance_report"]["tables"].split('.')[1],
    date_range=(range_start, range_end),
    env=env,
    dag=the_dag,
    task_id='waiting_for_age_performance_report_load_table_to_complete')

gender_external_dependency = DataAvailabilitySensor(
    schema=config["feeds"]["gender_performance_report"]["tables"].split('.')[0],
    table_name=config["feeds"]["gender_performance_report"]["tables"].split('.')[1],
    date_range=(range_start, range_end),
    env=env,
    dag=the_dag,
    task_id='waiting_for_gender_performance_report_load_table_to_complete')

user_external_dependency = DataAvailabilitySensor(
    schema=config["feeds"]["user_performance_report"]["tables"].split('.')[0],
    table_name=config["feeds"]["user_performance_report"]["tables"].split('.')[1],
    date_range=(range_start, range_end),
    env=env,
    dag=the_dag,
    task_id='waiting_for_user_performance_report_load_table_to_complete')

topic_external_dependency = DataAvailabilitySensor(
    schema=config["feeds"]["topic_performance_report"]["tables"].split('.')[0],
    table_name=config["feeds"]["topic_performance_report"]["tables"].split('.')[1],
    date_range=(range_start, range_end),
    env=env,
    dag=the_dag,
    task_id='waiting_for_topic_performance_report_load_table_to_complete')

webpage_external_dependency = DataAvailabilitySensor(
    schema=config["feeds"]["webpage_performance_report"]["tables"].split('.')[0],
    table_name=config["feeds"]["webpage_performance_report"]["tables"].split('.')[1],
    date_range=(range_start, range_end),
    env=env,
    dag=the_dag,
    task_id='waiting_for_webpage_performance_report_load_table_to_complete')

keyword_criteria_external_dependency = DataAvailabilitySensor(
    schema=config["feeds"]["keyword_criteria_performance_report"]["tables"].split('.')[0],
    table_name=config["feeds"]["keyword_criteria_performance_report"]["tables"].split('.')[1],
    date_range=(range_start, range_end),
    env=env,
    dag=the_dag,
    task_id='waiting_for_keyword_criteria_performance_report_load_table_to_complete')

parent_external_dependency = DataAvailabilitySensor(
    schema=config["feeds"]["parent_performance_report"]["tables"].split('.')[0],
    table_name=config["feeds"]["parent_performance_report"]["tables"].split('.')[1],
    date_range=(range_start, range_end),
    env=env,
    dag=the_dag,
    task_id='waiting_for_parent_performance_report_load_table_to_complete')

income_external_dependency = DataAvailabilitySensor(
    schema=config["feeds"]["income_performance_report"]["tables"].split('.')[0],
    table_name=config["feeds"]["income_performance_report"]["tables"].split('.')[1],
    date_range=(range_start, range_end),
    env=env,
    dag=the_dag,
    task_id='waiting_for_income_performance_report_load_table_to_complete')

placement_external_dependency = DataAvailabilitySensor(
    schema=config["feeds"]["placement_performance_report"]["tables"].split('.')[0],
    table_name=config["feeds"]["placement_performance_report"]["tables"].split('.')[1],
    date_range=(range_start, range_end),
    env=env,
    dag=the_dag,
    task_id='waiting_for_placement_performance_report_load_table_to_complete')

spark_task = ZacsSparkSubmitOperator(
    task_id="load_criteria_performance",
    zodiac_environment=env,
...
hive_tables = config["common"]["tables_criteria"]
entity_uid = config['feeds']["googleads_criteria_performance"]['lcp']["entity_uid"]
